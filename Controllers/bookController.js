...
export const getBooks = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ ok: true, message: "REQUEST SUCCESSFULL", data: AllBooks });

    if (!AllBooks) {
      return res
        .status(400)
        .json({ ok: false, message: "Missing Input, Book Required" });
    }
  } catch (error) {
...
export const getBooksById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ ok: true, message: "Request successful", data: book });
  } catch (error) {
    res.status(500).json({ ok: false, error: error.message });
  }
};
